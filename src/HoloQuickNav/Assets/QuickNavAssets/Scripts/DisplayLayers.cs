...
public class DisplayLayers : MonoBehaviour {
    ...
    void Start () {
        ...
        numberOfLayers = GameObject.Find("Model").transform.FindChild("Layers").transform.childCount;
    ...
    {
        Transform buttonPrefab = GameObject.Find("Controls").transform.FindChild("Home/LayerSelector/Buttons/ButtonPrefab").transform;
        ...
        for(int i = 0; i< (numberOfLayers-1); i++)
        {
            ...
            newButton.parent = GameObject.Find("Controls").transform.FindChild("Home/LayerSelector/Buttons").transform;
        }
        ...
        {
            layerName = GameObject.Find("Model").transform.FindChild("Layers").transform.GetChild(i).name;
            //shorten name if needed to fit on button
            if(layerName.Length > 12)
            {
                layerName = layerName.Substring(0, 12) + "..";
            }
            button = GameObject.Find("Controls").transform.FindChild("Home/LayerSelector/Buttons").transform.GetChild(i).gameObject;
    ...
    private void PlaceMenu()
    {
        ...
        GameObject.Find("Controls").transform.FindChild("Home/LayerSelector/Buttons").transform.position += new Vector3(0f, shiftDistance, 0f);

        //scale menu background
        float backgroundHeight = (numberOfLayers * buttonDistance + (numberOfLayers - 1) * buttonHeight) + 0.06f;
        GameObject.Find("Controls").transform.FindChild("Home/LayerSelector/Background").transform.localScale = new Vector3(0.25f, backgroundHeight, 0.01f);
    }
    ...
    public void ToggleLayer(GameObject pressedButton)
    {
        ...
        if (timer > 2) //if 2 seconds have past since you last turned a layer on
        {
            ...
            if (GameObject.Find("Model").transform.FindChild("Layers").transform.GetChild(layerIndex).gameObject.activeSelf)
            {
                GameObject.Find("Model").transform.FindChild("Layers").transform.GetChild(layerIndex).gameObject.SetActive(false);
                matList[0] = buttonOff;
                pressedButton.transform.GetChild(0).GetComponent<MeshRenderer>().materials = matList;
            }
            else
            {
                GameObject.Find("Model").transform.FindChild("Layers").transform.GetChild(layerIndex).gameObject.SetActive(true);
                matList[0] = buttonOn;
                pressedButton.transform.GetChild(0).GetComponent<MeshRenderer>().materials = matList;
            }
        }      
    ...
    private void ResetButtons()
    {
        ...
        for (int i= 0;i< numberOfLayers;i++) 
        {
            button = GameObject.Find("Controls").transform.FindChild("Home/LayerSelector/Buttons").transform.GetChild(i).transform.GetChild(0).gameObject;

            if (GameObject.Find("Model").transform.FindChild("Layers").transform.GetChild(i).gameObject.activeSelf) //if layer visible
            {
                matList[0] = buttonOn;
